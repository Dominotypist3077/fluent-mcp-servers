...
import { errorFromStatus, FluentSupportApiError } from './errors.js'
...
async function parseSuccessBody<T>(response: Response, method: string, path: string): Promise<T> {
	...
	if (raw.trim().startsWith('<')) {
		...
		const message =
			typeof recoveredObj?.message === 'string'
				...
				: 'Received HTML instead of JSON'
		throw new FluentSupportApiError('CONNECTION_ERROR', message, response.status, {
			recovered,
	...
	throw new FluentSupportApiError(
		'CONNECTION_ERROR',
...
export type FluentSupportClient = ReturnType<typeof createClient>
