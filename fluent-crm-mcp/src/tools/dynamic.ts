...
function inferCategory(toolName: string): Category {
	const name = toolName.replace(/^fluentcrm_/, '')
	...
	if (name.startsWith('template')) return 'template'
	if (name.startsWith('sequence')) return 'sequence'
	if (name.startsWith('funnel')) return 'funnel'
...
export function registerDynamicTools(server: McpServer, tools: ToolDefinition[]): void {
	...
	// 1. fluentcrm_search_tools
	server.registerTool(
		'fluentcrm_search_tools',
		{
			title: 'Search FluentCRM Tools',
			description:
				'Search available FluentCRM tools by keyword and optional category. Returns matching tool names, titles, and descriptions. Use this to discover which tools are available before calling them.',
	...
	// 2. fluentcrm_describe_tools
	server.registerTool(
		'fluentcrm_describe_tools',
		{
			title: 'Describe FluentCRM Tools',
			description:
	...
	// 3. fluentcrm_execute_tool
	server.registerTool(
		'fluentcrm_execute_tool',
		{
			title: 'Execute FluentCRM Tool',
			description:
				'Execute a FluentCRM tool by name with the given input. Use describe_tools first to learn the required input schema.',
			inputSchema: z.object({
			...
			if (!tool) {
				return {
					content: [
						{
							type: 'text' as const,
							text: `Error: Tool "${args.tool_name}" not found. Use fluentcrm_search_tools to discover available tools.`,
						},
					],
					isError: true,
				}
