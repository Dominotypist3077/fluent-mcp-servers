...
import { FluentCrmApiError } from '../api/errors.js'
...
interface CustomToolConfig extends BaseToolConfig {
	handler: (client: FluentCrmClient, input: Record<string, unknown>) => Promise<unknown>
}
	...
	const arrayKey =
		'data' in obj && Array.isArray(obj.data)
			...
			: 'items' in obj && Array.isArray(obj.items)
				? 'items'
				: 'subscribers' in obj && Array.isArray(obj.subscribers)
					? 'subscribers'
					: 'contacts' in obj && Array.isArray(obj.contacts)
						? 'contacts'
						: 'campaigns' in obj && Array.isArray(obj.campaigns)
							? 'campaigns'
							: 'tags' in obj && Array.isArray(obj.tags)
								? 'tags'
								: 'lists' in obj && Array.isArray(obj.lists)
									? 'lists'
									: null
...
export function createTool(client: FluentCrmClient, config: CustomToolConfig): ToolDefinition {
	return {
		name: config.name,
		title: config.title,
...
export const getTool = (client: FluentCrmClient, config: EndpointToolConfig): ToolDefinition =>
	createEndpointTool(client, 'GET', config)

export const postTool = (client: FluentCrmClient, config: EndpointToolConfig): ToolDefinition =>
	createEndpointTool(client, 'POST', config)

export const putTool = (client: FluentCrmClient, config: EndpointToolConfig): ToolDefinition =>
