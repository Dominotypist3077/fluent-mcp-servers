...
import { errorFromStatus, FluentCommunityApiError } from './errors.js'
...
async function parseSuccessBody<T>(response: Response, method: string, path: string): Promise<T> {
	...
	if (raw.trim().startsWith('<')) {
		...
		const message =
			typeof recoveredObj?.message === 'string'
				...
				: 'Received HTML instead of JSON'
		throw new FluentCommunityApiError('CONNECTION_ERROR', message, response.status, {
			recovered,
	...
	throw new FluentCommunityApiError(
		'CONNECTION_ERROR',
...
export type FluentCommunityClient = ReturnType<typeof createClient>
