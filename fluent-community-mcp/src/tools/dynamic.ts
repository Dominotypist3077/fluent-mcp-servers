...
function inferCategory(toolName: string): Category {
	const name = toolName.replace(/^fluentcommunity_/, '')
	if (name.startsWith('space')) return 'space'
	if (name.startsWith('feed') || name.startsWith('post')) return 'feed'
	if (name.startsWith('comment')) return 'comment'
	if (name.startsWith('member')) return 'member'
	if (name.startsWith('course')) return 'course'
	if (name.startsWith('notification')) return 'notification'
	if (name.startsWith('analytics') || name.startsWith('stats')) return 'analytics'
...
export function registerDynamicTools(server: McpServer, tools: ToolDefinition[]): void {
	...
	// 1. fluentcommunity_search_tools
	server.registerTool(
		'fluentcommunity_search_tools',
		{
			title: 'Search Fluent Community Tools',
			description:
				'Search available Fluent Community tools by keyword and optional category. Returns matching tool names, titles, and descriptions. Use this to discover which tools are available before calling them.',
	...
	// 2. fluentcommunity_describe_tools
	server.registerTool(
		'fluentcommunity_describe_tools',
		{
			title: 'Describe Fluent Community Tools',
			description:
	...
	// 3. fluentcommunity_execute_tool
	server.registerTool(
		'fluentcommunity_execute_tool',
		{
			title: 'Execute Fluent Community Tool',
			description:
				'Execute a Fluent Community tool by name with the given input. Use describe_tools first to learn the required input schema.',
			inputSchema: z.object({
			...
			if (!tool) {
				return {
					content: [
						{
							type: 'text' as const,
							text: `Error: Tool "${args.tool_name}" not found. Use fluentcommunity_search_tools to discover available tools.`,
						},
					],
					isError: true,
				}
