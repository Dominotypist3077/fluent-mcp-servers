...
import { FluentCommunityApiError } from '../api/errors.js'
...
interface CustomToolConfig extends BaseToolConfig {
	handler: (client: FluentCommunityClient, input: Record<string, unknown>) => Promise<unknown>
}
	...
	const arrayKey =
		'data' in obj && Array.isArray(obj.data)
			...
			: 'items' in obj && Array.isArray(obj.items)
				? 'items'
				: 'feeds' in obj && Array.isArray(obj.feeds)
					? 'feeds'
					: 'spaces' in obj && Array.isArray(obj.spaces)
						? 'spaces'
						: 'members' in obj && Array.isArray(obj.members)
							? 'members'
							: 'comments' in obj && Array.isArray(obj.comments)
								? 'comments'
								: 'courses' in obj && Array.isArray(obj.courses)
									? 'courses'
									: 'notifications' in obj && Array.isArray(obj.notifications)
										? 'notifications'
										: null
...
function formatError(error: unknown) {
	if (error instanceof FluentCommunityApiError) {
...
export function createTool(client: FluentCommunityClient, config: CustomToolConfig): ToolDefinition {
	return {
		name: config.name,
		title: config.title,
...
export const getTool = (client: FluentCommunityClient, config: EndpointToolConfig): ToolDefinition =>
	createEndpointTool(client, 'GET', config)

export const postTool = (client: FluentCommunityClient, config: EndpointToolConfig): ToolDefinition =>
	createEndpointTool(client, 'POST', config)

export const putTool = (client: FluentCommunityClient, config: EndpointToolConfig): ToolDefinition =>
	createEndpointTool(client, 'PUT', config)

export const patchTool = (client: FluentCommunityClient, config: EndpointToolConfig): ToolDefinition =>
