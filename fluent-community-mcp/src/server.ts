...
export function resolveServerContext(): ServerContext {
	...
	const configSource = process.env.FLUENTCOMMUNITY_URL ? 'env' : 'file'
...
export function createServerFromContext(
	...
	console.error(
		`fluent-community-mcp v${ctx.version} started — ${toolCount} tools registered (${mode} mode)`,
	)
