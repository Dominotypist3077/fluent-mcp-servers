...
import { errorFromStatus, FluentBoardsApiError } from './errors.js'
...
async function parseSuccessBody<T>(response: Response, method: string, path: string): Promise<T> {
	...
	if (raw.trim().startsWith('<')) {
		...
		const message =
			typeof recoveredObj?.message === 'string'
				...
				: 'Received HTML instead of JSON'
		throw new FluentBoardsApiError('CONNECTION_ERROR', message, response.status, {
			recovered,
	...
	throw new FluentBoardsApiError(
		'CONNECTION_ERROR',
...
export type FluentBoardsClient = ReturnType<typeof createClient>
