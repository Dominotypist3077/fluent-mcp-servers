...
export function resolveServerContext(): ServerContext {
	...
	const configSource = process.env.FLUENTBOARDS_URL ? 'env' : 'file'
...
export function createServerFromContext(
	...
	console.error(
		`fluent-boards-mcp v${ctx.version} started — ${toolCount} tools registered (${mode} mode)`,
	)
