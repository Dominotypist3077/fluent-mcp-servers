...
import { FluentBoardsApiError } from '../api/errors.js'
...
interface CustomToolConfig extends BaseToolConfig {
	handler: (client: FluentBoardsClient, input: Record<string, unknown>) => Promise<unknown>
}
			...
			: 'items' in obj && Array.isArray(obj.items)
				...
				: 'tasks' in obj && Array.isArray(obj.tasks)
					...
					: 'boards' in obj && Array.isArray(obj.boards)
						? 'boards'
						: 'stages' in obj && Array.isArray(obj.stages)
							? 'stages'
							: 'labels' in obj && Array.isArray(obj.labels)
								? 'labels'
...
function formatError(error: unknown) {
	if (error instanceof FluentBoardsApiError) {
...
export function createTool(client: FluentBoardsClient, config: CustomToolConfig): ToolDefinition {
	return {
		name: config.name,
		title: config.title,
...
export const getTool = (client: FluentBoardsClient, config: EndpointToolConfig): ToolDefinition =>
	createEndpointTool(client, 'GET', config)

export const postTool = (client: FluentBoardsClient, config: EndpointToolConfig): ToolDefinition =>
	createEndpointTool(client, 'POST', config)

export const putTool = (client: FluentBoardsClient, config: EndpointToolConfig): ToolDefinition =>
