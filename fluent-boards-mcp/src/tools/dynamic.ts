...
function inferCategory(toolName: string): Category {
	const name = toolName.replace(/^fluentboards_/, '')
...
export function registerDynamicTools(server: McpServer, tools: ToolDefinition[]): void {
	...
	// 1. fluentboards_search_tools
	server.registerTool(
		'fluentboards_search_tools',
		{
			title: 'Search Fluent Boards Tools',
			description:
				'Search available Fluent Boards tools by keyword and optional category. Returns matching tool names, titles, and descriptions. Use this to discover which tools are available before calling them.',
	...
	// 2. fluentboards_describe_tools
	server.registerTool(
		'fluentboards_describe_tools',
		{
			title: 'Describe Fluent Boards Tools',
			description:
	...
	// 3. fluentboards_execute_tool
	server.registerTool(
		'fluentboards_execute_tool',
		{
			title: 'Execute Fluent Boards Tool',
			description:
				'Execute a Fluent Boards tool by name with the given input. Use describe_tools first to learn the required input schema.',
			inputSchema: z.object({
			...
			if (!tool) {
				return {
					content: [
						{
							type: 'text' as const,
							text: `Error: Tool "${args.tool_name}" not found. Use fluentboards_search_tools to discover available tools.`,
						},
					],
					isError: true,
				}
