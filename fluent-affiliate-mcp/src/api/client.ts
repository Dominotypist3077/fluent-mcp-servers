...
import { errorFromStatus, FluentAffiliateApiError } from './errors.js'
...
async function parseSuccessBody<T>(response: Response, method: string, path: string): Promise<T> {
	...
	if (raw.trim().startsWith('<')) {
		...
		const message =
			typeof recoveredObj?.message === 'string'
				...
				: 'Received HTML instead of JSON'
		throw new FluentAffiliateApiError('CONNECTION_ERROR', message, response.status, {
			recovered,
	...
	throw new FluentAffiliateApiError(
		'CONNECTION_ERROR',
...
export type FluentAffiliateClient = ReturnType<typeof createClient>
