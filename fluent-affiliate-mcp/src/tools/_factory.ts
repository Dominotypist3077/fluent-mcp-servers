...
import { FluentAffiliateApiError } from '../api/errors.js'
...
interface EndpointToolConfig extends BaseToolConfig {
	...
	/** Cache keys to invalidate on successful write (POST/PUT/PATCH/DELETE). */
	invalidates?: string[]
}

interface CustomToolConfig extends BaseToolConfig {
	handler: (client: FluentAffiliateClient, input: Record<string, unknown>) => Promise<unknown>
}
	...
	const arrayKey =
		'data' in obj && Array.isArray(obj.data)
			...
			: 'items' in obj && Array.isArray(obj.items)
				? 'items'
				: 'affiliates' in obj && Array.isArray(obj.affiliates)
					? 'affiliates'
					: 'referrals' in obj && Array.isArray(obj.referrals)
						? 'referrals'
						: 'payouts' in obj && Array.isArray(obj.payouts)
							? 'payouts'
							: 'visits' in obj && Array.isArray(obj.visits)
								? 'visits'
								: null
...
function formatError(error: unknown) {
	if (error instanceof FluentAffiliateApiError) {
...
export function createTool(client: FluentAffiliateClient, config: CustomToolConfig): ToolDefinition {
	return {
		name: config.name,
		title: config.title,
				...
				const fetcher = async () => {
					...
					switch (method) {
						...
						case 'PUT':
							...
							break
						case 'PATCH':
							response = await client.patch(path, rest)
							break
						case 'DELETE':
...
export const getTool = (client: FluentAffiliateClient, config: EndpointToolConfig): ToolDefinition =>
	createEndpointTool(client, 'GET', config)

export const postTool = (client: FluentAffiliateClient, config: EndpointToolConfig): ToolDefinition =>
	createEndpointTool(client, 'POST', config)

export const putTool = (client: FluentAffiliateClient, config: EndpointToolConfig): ToolDefinition =>
	createEndpointTool(client, 'PUT', config)

export const patchTool = (client: FluentAffiliateClient, config: EndpointToolConfig): ToolDefinition =>
	createEndpointTool(client, 'PATCH', config)
