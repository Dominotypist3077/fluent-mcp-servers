...
function inferCategory(toolName: string): Category {
	const name = toolName.replace(/^fluentaffiliate_/, '')
	if (name.startsWith('affiliate')) return 'affiliate'
	if (name.startsWith('referral')) return 'referral'
	if (name.startsWith('payout')) return 'payout'
	if (name.startsWith('visit')) return 'visit'
...
export function registerDynamicTools(server: McpServer, tools: ToolDefinition[]): void {
	...
	// 1. fluentaffiliate_search_tools
	server.registerTool(
		'fluentaffiliate_search_tools',
		{
			title: 'Search FluentAffiliate Tools',
			description:
				'Search available FluentAffiliate tools by keyword and optional category. Returns matching tool names, titles, and descriptions. Use this to discover which tools are available before calling them.',
	...
	// 2. fluentaffiliate_describe_tools
	server.registerTool(
		'fluentaffiliate_describe_tools',
		{
			title: 'Describe FluentAffiliate Tools',
			description:
	...
	// 3. fluentaffiliate_execute_tool
	server.registerTool(
		'fluentaffiliate_execute_tool',
		{
			title: 'Execute FluentAffiliate Tool',
			description:
				'Execute a FluentAffiliate tool by name with the given input. Use describe_tools first to learn the required input schema.',
			inputSchema: z.object({
			...
			if (!tool) {
				return {
					content: [
						{
							type: 'text' as const,
							text: `Error: Tool "${args.tool_name}" not found. Use fluentaffiliate_search_tools to discover available tools.`,
						},
					],
					isError: true,
				}
