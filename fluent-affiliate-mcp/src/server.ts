...
export function resolveServerContext(): ServerContext {
	...
	const configSource = process.env.FLUENTAFFILIATE_URL ? 'env' : 'file'
...
export function createServerFromContext(
	...
	console.error(
		`fluent-affiliate-mcp v${ctx.version} started — ${toolCount} tools registered (${mode} mode)`,
	)
